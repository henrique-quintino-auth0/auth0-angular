...
import { filter, distinctUntilChanged, switchMap, mergeMap, concatMap, catchError, tap, takeUntil, map, take, pluck, first } from 'rxjs/operators';
...
class AuthService {
    ...
    getAccessTokenSilently(options) {
        return of(this.auth0Client).pipe(concatMap((client) => client.getTokenSilently(options)), tap(() => this.refreshState$.next()), catchError((error) => {
            this.errorSubject$.next(error);
    ...
    getAccessTokenWithPopup(options) {
        return of(this.auth0Client).pipe(concatMap((client) => client.getTokenWithPopup(options)), tap(() => this.refreshState$.next()), catchError((error) => {
            this.errorSubject$.next(error);
            return throwError(error);
        }));
    }
